...
// ✅ 正确 - 使用转义符
{
  "task": {
    "stopModal": {
      "warning": "停止后将向机器人发送停止指令，执行状态将变为\"已停止\"。"
    },
    "retryModal": {
      "hint": "将创建新的执行记录，任务状态将变为\"待执行\"并重新加入队列。"
    }
  },
  "queueMessage": {
    "requeueModal": {
      "description": "将消息重新入队后，消息状态将变为\"未消费-已生效\"，可以被重新消费。"
    }
  }
}
...
// ❌ 错误 - 使用中文引号或未转义的双引号
{
  "warning": "执行状态将变为"已停止"。",  // 中文引号，需要改为转义的英文引号
  "hint": "任务状态将变为"待执行"。"       // 中文引号，需要改为转义的英文引号
}
```

**转义规则说明：**
- JSON 字符串中的双引号必须使用 `\"` 转义
- 避免使用中文引号（""），统一使用转义的英文双引号（`\"`）
- 这样可以确保 JSON 文件格式正确，避免解析错误
