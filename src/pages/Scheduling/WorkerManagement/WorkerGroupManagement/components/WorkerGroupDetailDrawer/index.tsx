...
const WorkerGroupDetailDrawer: React.FC<WorkerGroupDetailDrawerProps> = ({
  ...
  const memberColumns = [
    ...
    { title: t('worker.table.status'), dataIndex: 'status', key: 'status', width: 90, render: (status: WorkerStatus | undefined) => { if (!status) return null; const config = statusConfig[status]; return <StatusDot color={config.color as StatusDotColor} label={config.text} />; } },
