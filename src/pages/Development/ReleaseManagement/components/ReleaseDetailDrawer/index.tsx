...
const ReleaseDetailDrawer: React.FC<ReleaseDetailDrawerProps> = ({
  ...
  const mockContents = useMemo(() => {
    ...
    return [
      { process_id: 'proc-001', process_name: 'SAP_ERP_Order_Processing_And_Fulfillment_Workflow_V3', version_number: 'v3.2.1', process_description: '该Process用于processing来自SAP ERP系统's 所有客户订单, 包括订单Validation, Inventory Check, 价格calculation, 折扣应用, 税费calculation, 物流分配, 发票generation以及客户Notification etc.完整's 端到端业务Process. 支持多币种, 多仓库, 多物流商's 复杂场景processing, 并集成了Abnormalprocessing and 自动Retry机制以确保Process's 高可靠性 and DataMon致性. ' },
      { process_id: 'proc-002', process_name: 'Inventory CheckProcess', version_number: 'v1.0.0', process_description: 'ScheduledCheck库存' },
      { process_id: 'proc-003', process_name: 'Monthly_Financial_Report_Generation_And_Distribution_Workflow', version_number: 'v2.1.0', process_description: '每M自动generation财务report并分发给相关部门负责人, 支持PDF and Excel双Format输出. ' },
      { process_id: 'proc-004', process_name: 'Customer_Onboarding_KYC_Verification_And_Account_Provisioning_Enterprise_Workflow_With_Compliance_Check', version_number: 'v1.3.0', process_description: '客户入网全Processautomation, 涵盖KYC身份Validation, 合规Check, 风控评估, 账户开通, 权限分配, 欢迎邮件发送及CRM系统同步 etc.环节, 支持多国家地区's 监管要求适配. ' },
      { process_id: 'proc-005', process_name: '邮件Notification', version_number: 'v1.0.2', process_description: '' },
      { process_id: 'proc-006', process_name: 'Automated_Invoice_Reconciliation_And_Payment_Processing_With_Multi_Currency_Support', version_number: 'v2.0.0', process_description: '自动对账 and 付款processingProcess, 支持多币种汇率Conversion, 发票Wed单匹配, Abnormal标记 and 人工审批, ERP回写及银企直连付款指令generation, 确保财务Data准确性 and 时效性. ' },
