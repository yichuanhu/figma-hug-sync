...
import { Tabs, TabPane, Form, Button, Toast, Typography, Tag, Empty } from '@douyinfe/semi-ui';
...
const FieldConfigPanel = ({ field, index, allFields, onSubmit, onCancel }: Props) => {
  const { t } = useTranslation();
  const [draft, setDraft] = useState<SchemeField | null>(field);
  ...
  if (!draft) {
    return (
      <div className="field-config-panel empty">
        <Empty
          title={<Text type="tertiary">未选择字段</Text>}
          description={<Text type="tertiary" size="small">点击左侧字段卡片以编辑配置</Text>}
        />
      </div>
    );
  }
    ...
    <div className="field-config-panel">
      <div className="panel-header">
        <div className="panel-title">
          <Title heading={6} style={{ margin: 0 }}>{t('requirements.scheme.builder.fieldConfig.title', { name: draft.label })}</Title>
          <Text type="tertiary" size="small">{draft.key} · {draft.type}</Text>
        </div>
        <div className="panel-actions">
          <Button icon={<X size={14} strokeWidth={2} />} theme="borderless" type="tertiary" size="small" onClick={onCancel}>
            {t('common.cancel')}
          </Button>
          <Button icon={<Save size={14} strokeWidth={2} />} theme="solid" type="primary" size="small" onClick={handleSave} disabled={!!validationError}>
            {t('common.save')}
          </Button>
        </div>
      </div>
      ...
      {validationError && (
        <div className="panel-error">{validationError}</div>
      )}
              ...
              {IS_TEXT.includes(draft.type) && (
                <>
                  <Form.InputNumber field="minLength" label="最小长度" initValue={draft.validation?.minLength}
                    onChange={(v) => updateValidation({ minLength: v as number })} min={0} style={{ width: '100%' }} />
                  <Form.InputNumber field="maxLength" label="最大长度" initValue={draft.validation?.maxLength}
                    onChange={(v) => updateValidation({ maxLength: v as number })} min={0} style={{ width: '100%' }} />
              ...
              {IS_NUMBER.includes(draft.type) && (
                <>
                  <Form.InputNumber field="min" label="最小值" initValue={draft.validation?.min}
                    onChange={(v) => updateValidation({ min: v as number })} style={{ width: '100%' }} />
                  <Form.InputNumber field="max" label="最大值" initValue={draft.validation?.max}
                    onChange={(v) => updateValidation({ max: v as number })} style={{ width: '100%' }} />
                </>
              )}
