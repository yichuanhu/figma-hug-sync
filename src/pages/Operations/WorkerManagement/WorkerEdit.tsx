...
import { Typography, Input, TextArea, Select, RadioGroup, Radio, Button, Card, Toast, Spin } from '@douyinfe/semi-ui';
...
const WorkerEdit = () => {
  ...
  if (loading) {
    return (
      <AppLayout>
        <div className="worker-form-loading">
          <Spin size="large" />
        </div>
      </AppLayout>
    );
    ...
    <AppLayout>
      <div className="worker-form">
        <div className="worker-form-header">
          <Title heading={3} className="worker-form-header-title">{t('worker.edit.title')}</Title>
          <Text type="tertiary">{t('worker.edit.description')}</Text>
        </div>
        <div className="worker-form-container">
          <Card title={t('worker.create.basicInfo')} className="worker-form-card">
            <div className="worker-form-card-content">
              <div className="worker-form-field">
                <Text strong className="worker-form-label">{t('worker.create.fields.workerName')} *</Text>
                <Input value={name} onChange={setName} showClear />
              </div>
              <div className="worker-form-field">
                <Text strong className="worker-form-label">{t('common.description')}</Text>
                <TextArea value={description} onChange={setDescription} rows={3} maxCount={500} />
              </div>
              <div className="worker-form-field">
                <Text strong className="worker-form-label">{t('worker.create.fields.priority')}</Text>
                <Select value={priority} onChange={(v) => setPriority(v as string)} className="worker-form-select-full">
              ...
              <div className="worker-form-field">
                <Text strong className="worker-form-label">{t('worker.edit.fields.receiveTasks')}</Text>
          ...
          <Card title={t('worker.create.runtimeConfig')} className="worker-form-card">
            <div className="worker-form-card-content">
              <div className="worker-form-field">
                <Text strong className="worker-form-label">{t('worker.create.fields.desktopType')} *</Text>
          ...
          <Card title={t('worker.create.connectionParams')} className="worker-form-card">
            <div className="worker-form-card-content">
              <div className="worker-form-field">
                <Text strong className="worker-form-label">{t('worker.create.fields.account')} *</Text>
                <Input value={username} onChange={setUsername} showClear />
              </div>
              <div className="worker-form-field">
                <Text strong className="worker-form-label">{t('worker.create.fields.password')}</Text>
                <Input mode="password" placeholder={t('worker.edit.fields.passwordPlaceholder')} value={password} onChange={setPassword} />
              </div>
              {desktopType === 'Console' && (
                <div className="worker-form-field">
                  <Text strong className="worker-form-label">{t('worker.edit.fields.unlockScreen')}</Text>
              ...
              {desktopType === 'NotConsole' && (
                <div className="worker-form-field">
                  <Text strong className="worker-form-label">{t('worker.create.fields.resolution')}</Text>
                  <Input value={displaySize} onChange={setDisplaySize} showClear />
                </div>
              )}
              <div className="worker-form-field">
                <Text strong className="worker-form-label">{t('worker.detail.fields.forceLogin')}</Text>
