...
const WorkerDeleteModal = ({ visible, onClose, workerData, onConfirm }: WorkerDeleteModalProps) => {
    ...
    <Modal
      title={
        <div className="worker-delete-modal-header">
          <IconAlertTriangle className="worker-delete-modal-header-icon" />
      ...
      <div className="worker-delete-modal-content">
        {hasPendingTasks ? (
          <div className="worker-delete-modal-alert">
            ...
            <Text type="danger">{t('worker.deleteModal.hasPendingTasks')}</Text>
          </div>
        ) : (
          <>
            <Text className="worker-delete-modal-message">{t('worker.deleteModal.confirmMessage', { name: workerData.name })}</Text>
            <Text type="tertiary">{t('worker.deleteModal.deleteWarning')}</Text>
          </>
        )}
      </div>
    </Modal>
