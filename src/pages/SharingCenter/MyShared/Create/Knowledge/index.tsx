...
const KnowledgeCreatePage = () => {
  ...
  const submit = async (values: any, publish: boolean) => {
    ...
    if (publish) {
      if (!coverFile) { Toast.warning(t('sharing.assetSupply.publish.coverRequired')); return; }
    ...
    <div className="ms-create-page">
      ...
      <div className="ms-create-body">
        <Form labelPosition="top" labelAlign="left" onSubmit={(v) => submit(v, true)} getFormApi={(api) => ((window as any).__knForm = api)}>
          ...
          />
          <Form.Select
            field="knowledgeType" label={t('sharing.assetSupply.create.fields.knowledgeType')} initValue="manual"
            optionList={[
              { value: 'manual', label: t('sharing.assetSupply.create.knowledgeTypes.manual') },
              { value: 'errorCode', label: t('sharing.assetSupply.create.knowledgeTypes.errorCode') },
              { value: 'bestPractice', label: t('sharing.assetSupply.create.knowledgeTypes.bestPractice') },
              { value: 'faq', label: t('sharing.assetSupply.create.knowledgeTypes.faq') },
            ]}
          />
          <Form.TagInput
            field="tags" label={t('sharing.assetSupply.create.fields.tags')}
            placeholder={t('sharing.assetSupply.create.fields.tagsPh')}
          />
          <Form.TextArea
          ...
          <Form.Slot label={t('sharing.assetSupply.create.fields.attachments')}>
            <Upload
              action="" customRequest={({ onSuccess }: any) => setTimeout(() => onSuccess?.({}), 200)}
              accept=".pdf,.xlsx,.xls,.doc,.docx,.png,.jpg,.jpeg"
              maxSize={10240}
            >
              <Button icon={<IconUpload />}>{t('sharing.assetSupply.create.selectAttachment')}</Button>
            </Upload>
            <div style={{ color: 'var(--semi-color-text-2)', fontSize: 12, marginTop: 6 }}>
              {t('sharing.assetSupply.create.fields.attachmentsHint')}
            </div>
          </Form.Slot>
