...
const generateMockLYProcessResponse = (index: number): LYProcessResponse => {
  ...
  const mockDependencies: LYProcessDependency[] = [
    ...
    { resource_id: `cred-${(index % 8) + 1}`, resource_name: ['Enterprise Email Credential', 'Database Connection Credential', 'Third-party API Credential', 'SSH Server Credential', 'Git Repository Credential', 'ERP System Credential', 'CRM System Credential', 'OA System Credential'][index % 8], resource_type: 'CREDENTIAL', source: 'AUTO_DETECTED', resource_value: '••••••••', status: 'ACTIVE', credential_type: ([2, 6].includes(index % 8) ? 'ASSIGNED_VALUE' : (index % 8 === 4 ? 'PERSONAL_REF' : 'FIXED_VALUE')) },
